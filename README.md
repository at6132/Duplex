...
- **Duplex-1.3-1.7B** -- first generation v1.1, 1.7B base size
...
**Duplex-1.3-1.7B:** Same Qwen3-1.7B backbone (frozen), plus three new trainable components:
...
python scripts/train.py --phase 2 --max_steps 20000 --resume checkpoints/duplex-1.3-1.7b/step_10000.pt
```

### 3. Evaluate
```bash
python scripts/evaluate.py --duplex_ckpt checkpoints/duplex-1.3-1.7b/final.pt --n_samples 200 --ablation
```
