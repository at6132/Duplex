"""
Chat-style demo: talk to Duplex-1.3-1.7B like ChatGPT.
Single conversation panel, no comparison. For normal back-and-forth chat.

Usage:
    python scripts/demo2.py --duplex_ckpt checkpoints/duplex-1.3-1.7b/final.pt
"""
...
def main():
    ...
    parser.add_argument("--duplex_ckpt", required=True, help="Path to Duplex checkpoint (e.g. checkpoints/duplex-1.3-1.7b/final.pt)")
    ...
    print("Loading Duplex-1.3-1.7B...")
    duplex_model = load_duplex_model(args.qwen_path, args.duplex_ckpt)
    print("Ready.\n")

    MODEL_CHOICES = ["Duplex-1.3-1.7B", "Qwen3-1.7B (Vanilla)"]
